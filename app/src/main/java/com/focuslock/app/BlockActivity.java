...
public class BlockActivity extends Activity {
    ...
    private String blockedPackage; private String reminder; private WebView card; private CountDownTimer timer; private long duration; private boolean ready;
    ...
    private WebView buildCard(){
        ...
        card.setWebViewClient(new WebViewClient(){
            ...
            @Override public void onPageFinished(WebView v,String url){v.postDelayed(()->{ready=true;update(remaining());},80L);}
        });
    ...
    private String pageHtml(String name){
        try(InputStream in=getAssets().open("lockcards/"+name+".html");ByteArrayOutputStream out=new ByteArrayOutputStream()){byte[] b=new byte[4096];int n;while((n=in.read(b))!=-1)out.write(b,0,n);String html=out.toString("UTF-8");return html.replace("</head>",bridge()+"</head>");}
        catch(Exception e){return "<html><head>"+bridge()+"</head><body style='background:#F7F5EF'></body></html>";}
    }
    ...
    private String chooseReminder(){
        long session=LockStore.lockedUntil(this,blockedPackage);android.content.SharedPreferences p=getSharedPreferences("focuslock_lock_card_reminders",MODE_PRIVATE);String key="reminder:"+blockedPackage;
        if(p.getLong(key+":session",Long.MIN_VALUE)==session){String[] r=RemoteConfigStore.reminders(this,REMINDERS);return r[Math.max(0,Math.min(r.length-1,p.getInt(key+":index",0)))];}
        String[] r=RemoteConfigStore.reminders(this,REMINDERS);int next=(p.getInt("last_reminder",-1)+1)%r.length;p.edit().putLong(key+":session",session).putInt(key+":index",next).putInt("last_reminder",next).apply();return r[next];
    }
    private void update(long ms){if(!ready||card==null)return;card.evaluateJavascript("if(window.focusLockSet){window.focusLockSet("+js(appName())+","+js(format(ms))+","+js(reminder)+");}",null);}
