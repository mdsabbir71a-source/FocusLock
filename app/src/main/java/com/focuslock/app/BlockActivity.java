...
public class BlockActivity extends Activity {
    ...
    private static final Map<String, String> CARD_HTML = new HashMap<>();
    ...
    private String pageHtml(){
        String name = LockStore.lockCardName(this, blockedPackage);
        synchronized (CARD_HTML) {
            String cached = CARD_HTML.get(name);
            if (cached != null) return cached;
        }
        try(InputStream in=getAssets().open("lockcards/"+name+".html");ByteArrayOutputStream out=new ByteArrayOutputStream()){
            ...
            synchronized (CARD_HTML) { CARD_HTML.put(name, html); }
            return html;
    ...
    private void bindLiveCard(){if(!ready||card==null)return;card.evaluateJavascript("if(window.focusLockStart){window.focusLockStart("+LockStore.lockedUntil(this,blockedPackage)+","+LockStore.lockDuration(this)+","+js(appName())+","+js(reminder)+");}",null);}
    ...
    private String chooseReminder(){return LockStore.nextLockReminder(this,RemoteConfigStore.reminders(this,REMINDERS));}
