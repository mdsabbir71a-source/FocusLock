...
public final class GardenLockOverlay {
    ...
    private static void showMain(Context context,String packageName){
        ...
        try{
            ...
            card.setWebViewClient(new WebViewClient(){
                ...
                @Override public void onPageFinished(WebView v,String url){ready=true;refresh();}
            });
    ...
    private static String pageHtml(Context c,String name){try(InputStream in=c.getAssets().open("lockcards/"+name+".html");ByteArrayOutputStream out=new ByteArrayOutputStream()){byte[] b=new byte[4096];int n;while((n=in.read(b))!=-1)out.write(b,0,n);return out.toString("UTF-8")+bridge();}catch(Exception e){return "<html><body style='background:#F7F5EF'></body></html>"+bridge();}}
    ...
    private static void refresh(){if(card==null||appContext==null||blockedPackage==null)return;if(!LockStore.isLocked(appContext,blockedPackage)){hideMain();return;}long left=Math.max(0L,LockStore.lockedUntil(appContext,blockedPackage)-System.currentTimeMillis());if(ready)card.evaluateJavascript("window.focusLockSet("+js(appName(appContext,blockedPackage))+","+js(format(left))+");",null);MAIN.postDelayed(GardenLockOverlay::refresh,1000L);}
