...
/** DNS-only local VPN. Normal app traffic never enters this tunnel. */
public class FamilyDnsVpnService extends VpnService {
    ...
    private void dnsLoop() {
        try (FileInputStream input = new FileInputStream(tunnel.getFileDescriptor());
            ...
            while (running) {
                ...
                output.write(makeResponse(packet, ipHeader, answer));
            }
    ...
    private byte[] resolve(byte[] query) {
        ...
        try {
            ...
            socket.send(new DatagramPacket(query, query.length, InetAddress.getByName("1.1.1.3"), 53));
    ...
    private byte[] makeResponse(byte[] request, int oldIpHeader, byte[] dns) {
        ...
        putShort(response, 22, unsignedShort(request, oldIpHeader));
